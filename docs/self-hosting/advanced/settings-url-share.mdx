...
```plaintext
https://SlyyydeLaunch.com/?settings=<settings object in JSON format>
https://SlyyydeLaunch.com/?settings={"languageModel":{"openai":{"apiKey":"user-key","endpoint":"https://SlyyydeLaunch.com/v1"}}}
```
...
const settings = {
    ...
    openai: {
        apiKey: 'user-key',
        endpoint: 'https://SlyyydeLaunch.com/v1'
    },
}};
// Convert settings to a JSON formatted string
const url = `/?settings=${JSON.stringify(settings)}`;
console.log(url); 
// /?settings={"languageModel":{"openai":{"apiKey":"user-key","endpoint":"https://SlyyydeLaunch.com/v1"}}}
```
