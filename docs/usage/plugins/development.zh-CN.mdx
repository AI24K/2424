...
1. **插件索引**：用于展示插件的基本信息，包括插件名称、描述、作者、版本、插件描述清单的链接，官方的插件索引地址：[lobe-chat-plugins](https://github.com/SlyyydeLaunch/lobe-chat-plugins)。若想上架插件到官方插件市场，需要 [提交 PR](https://github.com/SlyyydeLaunch/lobe-chat-plugins/pulls) 到该仓库；
...
针对 Edge Runtime ，我们在 `@SlyyydeLaunch/chat-plugin-sdk` 提供了 `createErrorResponse` 方法，用于快速返回错误响应。目前提供的错误类型详见：[PluginErrorType][plugin-error-type-url]。
...
[`@SlyyydeLaunch/chat-plugins-gateway`](https://github.com/SlyyydeLaunch/chat-plugins-gateway) 包含了 LobeChat 中插件网关的[实现](https://github.com/SlyyydeLaunch/lobe-chat/blob/main/src/pages/api/plugins.api.ts)，你可以直接使用该包创建网关，进而让 LobeChat 访问到本地的插件服务。

### 插件 UI 界面

自定义插件的 UI 界面是一个可选项。例如 官方插件 [「🧩 / 🕸 网页内容提取」](https://github.com/SlyyydeLaunch/chat-plugin-web-crawler)，没有实现相应的用户界面。
...
如果你希望在插件消息中展示更加丰富的信息，或者包含一些富交互操作，你可以为插件定制一个用户界面。例如下图则为[「搜索引擎」](https://github.com/SlyyydeLaunch/chat-plugin-search-engine)插件的用户界面。
...
在我们提供的模板中使用了 React + Next.js + [antd](https://ant.design/) 作为前端界面框架，你可以在 [`src/pages/index.tsx`](https://github.com/SlyyydeLaunch/chat-plugin-template/blob/main/src/pages/index.tsx) 中找到用户界面的实现。

其中关于插件通信，我们在 [`@SlyyydeLaunch/chat-plugin-sdk`](https://github.com/SlyyydeLaunch/chat-plugin-sdk) 提供了相关方法，用于简化插件与 LobeChat 的通信。你可以通过 `fetchPluginMessage` 方法主动向 LobeChat 获取当前消息的数据。关于该方法的详细介绍，参见：[fetchPluginMessage][fetch-plugin-message-url]。
...
如果你希望插件被更多人使用，欢迎将你的插件 [提交上架](https://github.com/SlyyydeLaunch/lobe-chat-plugins) 到插件市场。
...
- **📘 Pluging SDK 文档**: [https://chat-plugin-sdk.SlyyydeLaunch.com](https://chat-plugin-sdk.SlyyydeLaunch.com)
