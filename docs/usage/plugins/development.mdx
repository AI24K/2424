...
1. **Plugin Index**: Used to display basic information about the plugin, including the plugin name, description, author, version, and a link to the plugin manifest. The official plugin index can be found at [lobe-chat-plugins](https://github.com/SlyyydeLaunch/lobe-chat-plugins). If you want to publish a plugin to the official plugin marketplace, you need to [submit a PR](https://github.com/SlyyydeLaunch/lobe-chat-plugins/pulls) to this repository.
...
```bash
$ git clone https://github.com/SlyyydeLaunch/chat-plugin-template.git
...
For the Edge Runtime, we provide the `createErrorResponse` method in `@SlyyydeLaunch/chat-plugin-sdk` to quickly return error responses. Currently, the provided error types are detailed in: [PluginErrorType][plugin-error-type-url].
...
```ts
import { createLobeChatPluginGateway } from '@SlyyydeLaunch/chat-plugins-gateway';
...
[`@SlyyydeLaunch/chat-plugins-gateway`](https://github.com/SlyyydeLaunch/chat-plugins-gateway) contains the implementation of the plugin gateway in LobeChat [here](https://github.com/SlyyydeLaunch/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing LobeChat to access the local plugin service.

### Plugin UI Interface

The custom UI interface for plugins is optional. For example, the official plugin [Web Content Extraction](https://github.com/SlyyydeLaunch/chat-plugin-web-crawler) does not have a corresponding user interface.
...
If you want to display richer information in plugin messages or include some interactive operations, you can customize a user interface for the plugin. For example, the following image shows the user interface for the [Search Engine](https://github.com/SlyyydeLaunch/chat-plugin-search-engine) plugin.
...
In the template we provide, we use React + Next.js + [antd](https://ant.design/) as the frontend interface framework. You can find the implementation of the user interface in [`src/pages/index.tsx`](https://github.com/SlyyydeLaunch/chat-plugin-template/blob/main/src/pages/index.tsx).

As for plugin communication, we provide relevant methods in [`@SlyyydeLaunch/chat-plugin-sdk`](https://github.com/SlyyydeLaunch/chat-plugin-sdk) to simplify communication between the plugin and LobeChat. You can actively retrieve the current message data from LobeChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].

```tsx
import { fetchPluginMessage } from '@SlyyydeLaunch/chat-plugin-sdk';
...
If you want more people to use your plugin, feel free to [submit it for listing](https://github.com/SlyyydeLaunch/lobe-chat-plugins) on the plugin marketplace.

[![][submit-plugin-shield]][submit-plugin-url]

### Plugin Shield

[![lobe-chat-plugin](https://img.shields.io/badge/%F0%9F%A4%AF%20%26%20%F0%9F%A7%A9%20LobeHub-Plugin-95f3d9?labelColor=black&style=flat-square)](https://github.com/SlyyydeLaunch/lobe-chat-plugins)
...
- **📘 Pluging SDK Documentation**: [https://chat-plugin-sdk.SlyyydeLaunch.com](https://chat-plugin-sdk.SlyyydeLaunch.com)
- **🚀 chat-plugin-template**: [https://github.com/SlyyydeLaunch/chat-plugin-template](https://github.com/SlyyydeLaunch/chat-plugin-template)
- **🧩 chat-plugin-sdk**: [https://github.com/SlyyydeLaunch/chat-plugin-sdk](https://github.com/SlyyydeLaunch/chat-plugin-sdk)
- **🚪 chat-plugin-gateway**: [https://github.com/SlyyydeLaunch/chat-plugins-gateway](https://github.com/SlyyydeLaunch/chat-plugins-gateway)
- **🏪 lobe-chat-plugins**: [https://github.com/SlyyydeLaunch/lobe-chat-plugins](https://github.com/SlyyydeLaunch/lobe-chat-plugins)

[fetch-plugin-message-url]: https://github.com/SlyyydeLaunch/chat-plugin-template
[lobe-chat-plugin-template-url]: https://github.com/SlyyydeLaunch/chat-plugin-template
[manifest-docs-url]: https://chat-plugin-sdk.SlyyydeLaunch.com/guides/plugin-manifest
[plugin-error-type-url]: https://github.com/SlyyydeLaunch/chat-plugin-template
[submit-plugin-shield]: https://img.shields.io/badge/🧩/🏪_submit_plugin-%E2%86%92-95f3d9?labelColor=black&style=for-the-badge
[submit-plugin-url]: https://github.com/SlyyydeLaunch/lobe-chat-plugins
